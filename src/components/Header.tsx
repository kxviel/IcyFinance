...
const Header = ({ theme, toggleTheme, openCommands }: HeaderProps) => {
	...
	return (
		<header className="flex min-h-28 items-center gap-12 border-b border-border max-[1200px]:gap-6 max-[960px]:min-h-24 max-[960px]:flex-wrap max-[960px]:gap-4 max-[960px]:py-6 min-[1700px]:min-h-34 print:hidden">
			...
			<div className="flex items-center gap-5 max-[1200px]:gap-3 max-[960px]:ml-auto">
				...
				<button
					type="button"
					className="group size-8 overflow-hidden rounded-full border border-foreground p-0 max-[680px]:size-7"
					aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} theme`}
					onClick={toggleTheme}
				>
					<span className="ml-[50%] block h-full w-1/2 bg-foreground transition-transform group-hover:-translate-x-full" />
				</button>
				<IconButton
